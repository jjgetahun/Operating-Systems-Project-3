...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  p->handlers[SIGSEGV] = (sighandler_t) -1; //SIGSEGV
  p->restorer_addr = -1;
...
void signal_deliver(int signum, siginfo_t info)
{
	...
	*((uint*)(proc->tf->esp - 20)) = info.type;		        // signal number
    *((uint*)(proc->tf->esp - 24)) = info.addr;             // siginfo_t address
    *((uint*)(proc->tf->esp - 28)) = (uint)signum;          // siginfo_t type
...
int mprotect(void *addr, int len, int prot) {
    ...
    proc->info.addr = (uint)addr;
    proc->info.type = prot;

    void * new_addr;

    for (new_addr = (void *)PGROUNDDOWN((uint)addr); new_addr < addr+len; new_addr += PGSIZE) {
        
        pte_t * pt_entry = walkpgdir(proc->pgdir, new_addr, 1);
